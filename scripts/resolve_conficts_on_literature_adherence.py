...
for index, row in df.iterrows():
    ...
    vulnerability_in_exam = 'denial of service'
    # Check if Tag1 is different from Tag2 and if Tag is NaN
    if (row["IsInLiteratureTag1"] != row["IsInLiteratureTag2"] and pd.isna(row["IsInLiteratureTag"])
        ...
        print(
            f"Tag1: \033[94m{row['IsInLiteratureTag1']}\033[0m, Tag2: \033[93m{row['IsInLiteratureTag2']}\033[0m")  # Blue for Tag1, Yellow for Tag2
        ...
        if user_input == '1':
            df.at[index, 'IsInLiteratureTag'] = row["IsInLiteratureTag1"]
        elif user_input == '2':
            df.at[index, 'IsInLiteratureTag'] = row["IsInLiteratureTag2"]
        else:
